...
DEFINE_string(simstring_dir, "/tmp", "Store simstring db files in this directory");
DEFINE_string(product_db_name, "simstring_product", "File prefix for product simstring db");
DEFINE_string(ingredient_db_name, "simstring_ingred", "File prefix for ingred db");
DEFINE_bool(create_simstring_db, true, "If true, create simstring db again. If false, use"
                                       " existing db files.");
// Required!
...
DEFINE_string(recipe_in_extra, "", "Load additional recipe list from this file");
...
int main(int argc, char** argv) {
   ...
   if (FLAGS_recipe_in.empty() || FLAGS_product_in.empty()) {
      cerr << "Must supply recipe and product input files. Try --help for info\n";
      return 1;
   }
   ...
   while (true) {
      ...
      if (input_line == ".exit" || input_line == ".quit" || input_line == ".q") {
         ...
         continue;
      } else if (std::all_of(input_line.begin(), input_line.end(), ::isdigit)) {
         long int gtin;
      ...
      vector<string> matching_ingreds = GetSimstrings(
      ...
      cout << "Matching ingredients:\n";
      for (const string& ingred_name : matching_ingreds) {
         ...
         unordered_map<string, unordered_set<const zns::Recipe*>>::const_iterator
               ingred_iter = ingredient_to_recipes.find(ingred_name);
         const unordered_set<const zns::Recipe*>& recipes = ingred_iter->second;
