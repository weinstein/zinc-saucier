...
int main(int argc, char** argv) {
   ...
   unordered_set<string> all_categories;
   for (const zns::Recipe& recipe : recipe_list.entries()) {
      ...
      for (const string& category : recipe.categories()) {
   ...
   std::unordered_map<string, Perceptron> category_to_neuron;
   // features: ingredient presence, plus prep/cook time, plus bias(1)
   const int num_features = all_ingredients.size() + 2 + 1;
   for (const string& category : all_categories) {
      category_to_neuron.insert({category, Perceptron(num_features)});
   }
   // map ingredient name to feature index
   std::unordered_map<string, int> ingred_to_id;
   for (const string& ingredient : all_ingredients) {
      ingred_to_id.insert({ingredient, ingred_to_id.size()});
   }
   ...
   while (true) {
      ...
      if (input_line == ".exit" || input_line == ".quit" || input_line == ".q") {
      ...
      } else if (input_line == ".print_training") {
         int n_recipes = recipe_list.entries_size();
         std::uniform_int_distribution<int> uint_dist(0, n_recipes-1);
         std::uniform_int_distribution<int> bool_dist(0, 1);
         int rand_i = uint_dist(rng);
         zns::Recipe* rand_recipe = recipe_list.mutable_entries(rand_i);
         cout << rand_recipe->DebugString();
         for (const zns::Ingredient& ingred : rand_recipe->ingredients()) {
            bool include_ingred = bool_dist(rng);
            cout << "has_ingredient: " << (include_ingred ? "1" : "0") << "\n";
         }
      } else if (input_line == ".print_categories") {
         for (const string& category : all_categories) {
            cout << category << "\n";
         }
      } else if (input_line == ".classify_recipe") {
         std::vector<double> features(num_features);
         features[0] = 1;
         for (const unordered_set<const zns::Ingredient*>& ingred_set : user_inventory) {
            for (const zns::Ingredient* ingred : ingred_set) {
               std::unordered_map<string, int>::const_iterator
                     iter = ingred_to_id.find(ingred->name());
               if (iter != ingred_to_id.end()) {
                  cerr << iter->first << " -> " << iter->second << "\n";
                  features[3 + iter->second] = 1;
               }
            }
         }
         for (const auto& kv : category_to_neuron) {
            const string& cat = kv.first;
            const Perceptron& neuron = kv.second;
            cerr << "neron epoch: " << neuron.GetEpoch() << "\n";
            cout << neuron.GetActivation(features) << "\t" << cat << "\n";
         }
      } else if (input_line == ".train_categories") {
         // map from category name to correct/incorrect identifications
         std::unordered_map<string, int> n_correct;
         std::unordered_map<string, int> n_incorrect;
         int n_recipes = recipe_list.entries_size();
         std::uniform_int_distribution<int> uint_dist(0, n_recipes-1);
         std::uniform_int_distribution<int> bool_dist(0, 1);
         ...
         while (true) {
            ...
            for (int i = 0; i < recipe_list.entries_size(); ++i) {
               int rand_i = uint_dist(rng);
               const zns::Recipe& recipe = recipe_list.entries(rand_i);
               // form feature vector
               std::vector<double> features(num_features);
               for (const zns::Ingredient& ingredient : recipe.ingredients()) {
                  std::unordered_map<string, int>::const_iterator
                        iter = ingred_to_id.find(ingredient.name());
                  if (iter != ingred_to_id.end()) {
                     features[3 + iter->second] = 1;
                  }
               }
               features[0] = 1;
//               features[1] = recipe.prep_time_min();
//               features[2] = recipe.cook_time_min();

               std::unordered_set<string> cat_compl = all_categories;
               for (const string& category : recipe.categories()) {
                  std::unordered_map<string, Perceptron>::iterator iter =
                        category_to_neuron.find(category);
                  if (iter != category_to_neuron.end()) {
                     if (iter->second.GiveTrainingData(features, 1)) {
                        ++n_correct[category];
                     } else {
                        ++n_incorrect[category];
                     }
                  }
                  cat_compl.erase(category);
               }
               for (const string& category : cat_compl) {
                  std::unordered_map<string, Perceptron>::iterator iter =
                        category_to_neuron.find(category);
                  if (iter != category_to_neuron.end()) {
                     if (iter->second.GiveTrainingData(features, 0)) {
                        ++n_correct[category];
                     } else {
                        ++n_incorrect[category];
                     }
            ...
            for (auto& kv : category_to_neuron) {
               kv.second.EpochTick();
               const string& cat = kv.first;
               double acc = 1. * n_correct[cat] / (n_correct[cat] + n_incorrect[cat]);
               if (acc < 0.9) {
         ...
         cout << "Completed after " << epochs << " epochs.\n";
      }
   }
