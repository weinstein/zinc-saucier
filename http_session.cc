...
bool HttpSession::Get(const string& url, const vector<PostField>& fields,
                      string* hdrs, string* body) {
...
bool HttpSession::Post(const string& url, const vector<PostField>& post_data,
                       string* hdrs, string* body) {
   return Post(url, PostFieldsToString(post_data), hdrs, body);
}

bool HttpSession::Post(const string& url, const string& post_data,
                       string* hdrs, string* body) {
   if (!curl_) {
      return false;
   }
...
void HttpSession::AddCustomHeader(const string& key, const string& value) {
   string header_line;
   if (value.empty()) {
      header_line = key + ";";
   } else {
